...
public class EmpDAO {
	...
	public void empInsert(EmpVO vo)
	{
		try
		{
			...
			// JOIN -> SELECT
			// SUBQUERY -> SELECT, INSERT, UPDATE, DELETE 모두 사용 가능
			String sql="INSERT INTO myEmp VALUES("
					+ "(SELECT NVL(MAX(empno)+1,7000) FROM myEmp),"
					+ "?,?,?,SYSDATE,?,?,?)";
				// 자동증가수 이름 잡 엠지알 샐러리 커미션 부서번호
			ps=conn.prepareStatement(sql);
			// ?에 값을 채운다
			// setString() setInt() setDouble() setDate()
			// ----------                       --------- => 이 둘은 자동으로 ' ' 싱글따옴표가 붙는다 
			// => table명과 컬럼명에는 사용하지 않는다
...
//					+ "AND empno=?";
			String sql="SELECT empno,ename,job,NVL(mgr,0),TO_CHAR(hiredate,'YYYY-MM-DD'),TO_CHAR(sal,'L999,999'),NVL(comm,0),"
					+ "(SELECT dname FROM myDept md WHERE me.deptno=md.deptno),"
					+ "(SELECT loc FROM myDept md WHERE me.deptno=md.deptno),"
					+ "(SELECT grade FROM myGrade mg WHERE sal BETWEEN losal AND hisal) "
					+ "FROM myEmp me "
					+ "WHERE empno=?";
