...
public class FoodModel {
	...
	/*
	 ...
	 ---
	   1. DAO연결 => 오라클 연결만 담당.
	   2. DAO 가져오고 , 브라우저 띄우고(JSP) ->여기 연결해서 => 결과값받아오고 ==> MODEL
	   3. 브라우저로 연결하는 부분: 값을 전송하고 요청 => Controller (Front controller)
	      DispatcherServlet 
	   4. 전송받은 데이터를 출력 담당 : View(JSP)
	      --------------- request,session
	  -----------------------------------------------------------------------
	   동작
	   ...
	   2-1. DispatcherServlet이 요청내용을 받는다 
	      => service() (URI를 받아서 => .do 를 추출한다)
	   2-2. 요청 처리에 해당되는 메소드를 찾는다
	      => 어노테이션 기능 (@RequestMapping)
	         -------
	   ...
	   4. DispatcherServlet 
	      ...
	      => RequestDispatcher rd=request.getReqeustDispathcer(jsp)
	...
	public String food_list(HttpServletRequest request,HttpServletResponse response)
	{
		...
		request.setAttribute("list", list);
		// 전송 시 => return에 있는 jsp가 받는다~
		// include시에는 include의 모든 jsp에서 사용 가능하다
	...
	public String food_detail_before(HttpServletRequest request,HttpServletResponse response)
	{
		...
		// ---쿠키의 단점은? 문자열만 저장이 가능하다( 요청한 사용자의 브라우저로 전송)
		// 저장의 위치 결정
		cookie.setPath("/");
